...
class QueryDslJoinTest {
    ...
    void practice_1() {
        //given
        String targetGroupName = "아이브";
        //when
        List<Tuple> idolList = factory
                .select(idol, group)
                .from(idol)
                .innerJoin(idol.group, group)
                .where(idol.group.groupName.eq(targetGroupName))
                .fetch();
        //then
        assertFalse(idolList.isEmpty());
        System.out.println("\n\n\n");
        idolList.forEach(tuple -> {
            Idol foundIdol = tuple.get(idol);
    ...
    @Test
    @DisplayName("문제 2. 그룹별 평균 나이 계산하여 평균 나이가 22세 이상인 그룹의 그룹명과 평균나이 조회")
    void practice_2() {
        //given
    ...
    @Test
    @DisplayName("문제 3. 2022년에 발매된 앨범이 있는 아이돌의 이름과 그룹명과 앨범명과 발매년도 조회")
    void practice_3() {
        //given
        int year = 2022;
        //when
