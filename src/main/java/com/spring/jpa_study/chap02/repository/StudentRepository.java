package com.spring.jpa_study.chap02.repository;


import com.spring.jpa_study.chap02.entity.Student;
...
public interface StudentRepository extends JpaRepository<Student, String> {
    ...
    // where major like '%major%'  ex) 검색커리
...
//    List<Student> findByAgeLessThanEqual(int age);
    ...
    // 도시명으로 학생 1명을 단일 조회
    @Query(value="SELECT st FROM Student AS st WHERE st.city = ?1", nativeQuery = false)
    ...
    @Query("DELETE FROM Student AS s WHERE s.name = ?1 AND s.city = ?2")
